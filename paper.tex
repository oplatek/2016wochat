...
\OP{new client interface image so the text is visible, fit to 8 pages}
\OP{grammar check} \\
...
During the data collection process we discarded over \OP{40} \% of conversation because one of the utterances were mark as low quality.
Note that we discard only the low quality utterance and sufix of that conversation, the prefix is submitted for finishing the conversation in next rounds of jobs.
